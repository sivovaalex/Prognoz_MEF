...
/** Расчёт рейтинга ОМСУ ЗАТО:
 *  - только ОМСУ с isZato = true
 *  - только показатели с zato = true
 *  - формула ratingFormulaZato (из настроек показателя)
 */
export function computeZatoRating(state: AppState, mode: 'preview' | 'final', opts: RatingOptions = {}): MunRating[] {
  ...
  const rows: MunRating[] = zatoMuns.map((m) => {
    const cells: Record<string, CellData> = {};
    let score = 0;
    let missing = 0;
    inds.forEach((ind) => {
    ...
    return {
      ...
      name: m.name,
      cells,
      score: inds.length === 0 ? null : (missing === inds.length ? null : score),
      place: null,
      complete: missing === 0,
      missing,
    };
